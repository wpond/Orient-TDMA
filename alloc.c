...
/* variables */
ALLOC_Slot allocTable[255];
uint8_t slotCount,
	slotOffset,
	nextAck = 253;

/* prototypes */
void ALLOC_ResetSlot(ALLOC_Slot* slot);
uint8_t ALLOC_NextEmptySlot();
uint8_t ALLOC_NodeSlot(uint8_t id);
uint8_t ALLOC_NextAck();
bool ALLOC_Alloc(uint8_t slotId, uint8_t id, uint8_t lease);
void ALLOC_TraceTable();
...
void ALLOC_DecrementLeases()
{
	...
	for (i = slotOffset; i < slotOffset + slotCount; i++)
	{
		if (allocTable[i].lease > 0)
		{
			...
			if (allocTable[i].lease == 0)
			{
				if (i > slotOffset && allocTable[i-1].nodeId == allocTable[i].nodeId)
				{
					allocTable[i-1].ack = 253;
				}
				else if (i < slotOffset+slotCount+1 && allocTable[i+1].nodeId == allocTable[i].nodeId)
				{
					allocTable[i+1].ack = 253;
				}
			}
...
void ALLOC_RecvAck(PACKET_Raw *packet)
{
	...
	uint8_t ack = packet->payload[0];
	uint8_t i;
	for (i = slotOffset; i < slotOffset + slotCount; i++)
	{
		if (allocTable[i].ack == ack)
			allocTable[i].ack = 254;
	}
