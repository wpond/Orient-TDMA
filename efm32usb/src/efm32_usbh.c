 ...
 *
 * @param[in] wIndex
 *   Word sized field that varies according to request. Ttypically used to pass
 *   an index or offset.
 *
 * @param[in] wLength
 ...
 *   timeout and @ref USB_STATUS_DEVICE_MALFUNCTION if a device was attached
...
int USBH_WaitForDeviceConnectionB( uint8_t *buf, int timeoutInSeconds )
{
  ...
  while ( 1 )
  {
    ...
    if ( 8 == USBH_ControlMsgB( &device->ep0,
                    USB_SETUP_DIR_D2H | USB_SETUP_RECIPIENT_DEVICE |
                    USB_SETUP_TYPE_STANDARD_MASK,         /* bmRequestType */
                    GET_DESCRIPTOR,                       /* bRequest      */
                    USB_DEVICE_DESCRIPTOR << 8,           /* wValue        */
...
@page usb_host USB host stack library
  ...
  The source files for the USB host stack resides in the efm32usb directory
  and follows the naming convention: efm32_usbh<em>nnn</em>.c/h.
...
@n @section usb_host_intro Introduction
  ...
  The EFM32 USB host protocol stack provides an API which makes it possible to
