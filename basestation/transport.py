...
class Transport:
	...
	def recv(self,packet):
		...
		if not valid:
			...
			msg = "Expected %d/%d but received %d/%d (seg/frame)\n" % (
				expectedSegment,
				expectedFrame,
				segmentId,
				frameId)
			#self.parent.debugger.sendMsg(msg)
		...
		if valid:
			...
			s = ""
			if EoF:
				s += "FRAGMENT %d/%d EOF" % (segmentId,frameId)
			else:
				s += "FRAGMENT %d/%d" % (segmentId,frameId)
			'''
			print "==="
			...
			print "==="
			'''
			#self.parent.debugger.sendMsg(s)
