...
static int RunTest(uint16_t time, uint8_t nodeCount, int txp, uint8_t loss, bool optimisations, uint16_t dataRate, uint8_t slots, uint8_t lease,bool spreadspeed)
{
	...
	int increment = dataRate / nodeCount,
		curDataRate = increment;
	
	for (i = 1; i < nodeCount+1; i++)
	{
		EnableNode(i, txp, loss, optimisations, curDataRate, nodeCount + slots + 1);
		nodeInfo[i].dataRate = (float)curDataRate * 20.0f * 25.0f * 8.0f / 1024.0f;
		curDataRate += increment;
		Recv(100);
	}
