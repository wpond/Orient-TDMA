...
/* variables */
static NodeInfo_t nodeInfo[256];
...
int main(int argsc, char *argsv[])
{
	...
	{
		SERVER_SendMsg("unable to open COM port");
		printf("Unable to open COM%i\n",COMPORT+1);
		SERVER_Teardown();
		return 1;
	}
	
	SERVER_SendMsg("start");
		...
		if (n->enabled)
		{
			if (packet[3] == n->frame && packet[4] == n->seg)
			{
				if (countPackets)
				{
					...
					sprintf(msg,"recv:%i/%i/%i",packet[2],n->seg,n->frame);
					SERVER_SendMsg(msg);
				}
			...
			else
			{
				...
				{
					n->misses++;
					sprintf(msg,"miss:%i/%i/%i",packet[2],packet[4],packet[3]);
					SERVER_SendMsg(msg);
				}
			}
		}
