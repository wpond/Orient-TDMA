...
static int  UsbDataReceived(USB_Status_TypeDef status, uint32_t xferred,
                            uint32_t remaining);
static int USB_TransmitContinuousComplete(USB_Status_TypeDef status,
                              uint32_t xferred,
                              uint32_t remaining);
                           ...
                           uint32_t remaining)
{
  ...
  if ((status == USB_STATUS_OK) && (xferred > 0))
  {
    ...
    if (usbMessageLen >= MESSAGELENGTH) {
	    USB_Message++;
	  }
    usbRxIndex ^= 1;
                              ...
                              uint32_t remaining)
{
  ...
  if (status == USB_STATUS_OK)
  {
    USBD_Write(EP_DATA_IN, (void *)uartRxBuffer[0], xferred, USB_TransmitContinuousComplete);
  }
}

int USB_TransmitContinuous(uint8_t *buf, int len)
{
  if(usbOnline && (!usbActive))
  {
    usbActive = true;
    memcpy((void *)uartRxBuffer[0], (void *)buf, len);
    USBD_Write(EP_DATA_IN, (void *)uartRxBuffer[0], len, USB_TransmitContinuousComplete); 
  }
}
